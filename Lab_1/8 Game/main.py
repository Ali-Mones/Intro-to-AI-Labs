...
def check_move(tile):
    ...
    pos = (tile_rect.x - START_POINT[0]) // TILE_WIDTH + (
        tile_rect.y - START_POINT[1]
    ) // TILE_WIDTH * 3
    print("pos:", pos)
...
def set_state():
    for i in range(1, 9):
        tile_rect_pos = (tiles_rects[i].x - START_POINT[0]) // TILE_WIDTH + (
            tiles_rects[i].y - START_POINT[1]
        ) // TILE_WIDTH * 3
    ...
    if prev:
        ...
        for i in range(9):
            if cur_state[i] == "_":
                ...
                to_tile = (
                    START_POINT[0] + TILE_WIDTH * (i % 3),
                    START_POINT[1] + TILE_WIDTH * (i // 3),
                )
                moving = True
                return
    else:
...
def jumb_to_final():
    ...
    for ch in cur_state:
        if ch != "_":
            el = int(ch)
            tiles_rects[el].topleft = (
                stx + xstep * TILE_WIDTH,
                sty + ystep * TILE_WIDTH,
            )
        xstep += 1
        if xstep == 3:
            xstep = 0
...
tiles = {
    1: pygame.image.load(
        "Tiles/tile1_number_134_pixels/tile1_01_134.png"
    ).convert_alpha(),
    2: pygame.image.load(
        "Tiles/tile1_number_134_pixels/tile1_02_134.png"
    ).convert_alpha(),
    3: pygame.image.load(
        "Tiles/tile1_number_134_pixels/tile1_03_134.png"
    ).convert_alpha(),
    4: pygame.image.load(
        "Tiles/tile1_number_134_pixels/tile1_04_134.png"
    ).convert_alpha(),
    5: pygame.image.load(
        "Tiles/tile1_number_134_pixels/tile1_05_134.png"
    ).convert_alpha(),
    6: pygame.image.load(
        "Tiles/tile1_number_134_pixels/tile1_06_134.png"
    ).convert_alpha(),
    7: pygame.image.load(
        "Tiles/tile1_number_134_pixels/tile1_07_134.png"
    ).convert_alpha(),
    8: pygame.image.load(
        "Tiles/tile1_number_134_pixels/tile1_08_134.png"
    ).convert_alpha(),
}
...
while game_active:
    ...
    if play_mode == PLAY_GAME:
        if moving:
            ...
            pygame.draw.rect(
                background, pygame.Color("black"), (10, 10, 227, 580), width=5
            )

            # right BK
            pygame.draw.rect(background, (0, 0, 76), (690, 10, 280, 580))
            pygame.draw.rect(
                background, pygame.Color("black"), (690, 10, 280, 580), width=5
            )
            ...
            screen.blit(
                text_font.render(f"{nodes_expanded}", False, "Yellow"), (700, 200)
            )
            ...
            screen.blit(
                text_font.render(f"{time_spent} ms", False, "Yellow"), (700, 400)
            )
            ...
            if solve_bfs_text_rect.collidepoint(mouse_pos):
                solved, parents, explored, depth, nodes_expanded = ai_algorithms.BFS(
                    "".join(state)
                )
            elif solve_dfs_text_rect.collidepoint(mouse_pos):
                solved, parents, explored, depth, nodes_expanded = ai_algorithms.DFS(
                    "".join(state)
                )
                ...
                (
                    ...
                    depth,
                    nodes_expanded,
                ) = ai_algorithms.AStarManhattan("".join(state))
                ...
                (
                    ...
                    depth,
                    nodes_expanded,
                ) = ai_algorithms.AStarEuclidean("".join(state))
            end = time.time()
    ...
    # start screen
        ...
        if moving:
            ...
            if mouse.get_pressed()[0]:
                ...
                if selected != -1:
                    held_tile_rect.x = (
                        mouse_pos[0] - START_POINT[0]
                    ) // TILE_WIDTH * TILE_WIDTH + START_POINT[0]
                    held_tile_rect.y = (
                        mouse_pos[1] - START_POINT[1]
                    ) // TILE_WIDTH * TILE_WIDTH + START_POINT[1]
                    ...
                    if (
                        mouse_pos[0] >= START_POINT[0]
                        and mouse_pos[0] <= START_POINT[0] + 3 * TILE_WIDTH
                        and mouse_pos[1] >= START_POINT[1]
                        and mouse_pos[1] <= START_POINT[1] + 3 * TILE_WIDTH
                    ):
                        moving = True
                        selected = held_tile
                        to_tile = (
                            (mouse_pos[0] - START_POINT[0]) // TILE_WIDTH * TILE_WIDTH
                            + START_POINT[0],
                            (mouse_pos[1] - START_POINT[1]) // TILE_WIDTH * TILE_WIDTH
                            + START_POINT[1],
                        )
                    else:
